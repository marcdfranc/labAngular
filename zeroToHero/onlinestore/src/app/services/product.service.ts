...
export class ProductService {
     ...
     * @return Success
     */
    listProducts(category?: string | undefined, pageNumber?: number | undefined, pageSize?: number | undefined, httpContext?: HttpContext): Observable<ProductResponse[]> {
        let url_ = this.baseUrl + "/api/Product?";
        if (category === null)
    ...
    protected processListProducts(response: HttpResponseBase): Observable<ProductResponse[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
